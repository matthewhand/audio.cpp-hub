...
MODEL_ID="breeze-tts"
SERVICE_NAME="breeze"
WEIGHTS="/home/matthewh/audio.cpp/breeze/breeze-tts-2-q8_0.gguf"
BACKEND="${BACKEND:-hip}"
EXE="${EXE:-/home/matthewh/audio.cpp/build_hip/bin/audiocpp_server}"
THREADS="${THREADS:-10}"
DEVICE="${DEVICE:-1}"
EXECUTABLE_ID="${EXECUTABLE_ID:-vulkan}"
...
for ((i = 0; i < 60; i++)); do
  if curl -fsS --max-time 2 "$HUB_URL/api/models" >/dev/null; then
    break
  fi
  sleep 1
done
...
[[ -x "$EXE" ]] \
  || fatal "audiocpp_server executable is missing or not executable: $EXE"
[[ -f "$WEIGHTS" ]] \
  || fatal "Breeze weights are missing: $WEIGHTS"
...
if [[ -z "$line" ]]; then
  log "No existing Breeze instance; starting the $BACKEND child (device $DEVICE)"
  curl -fsS --max-time 15 -X POST "$HUB_URL/api/instances" \
    -H "Content-Type: application/json" \
    --data-raw "{\"modelId\":\"$MODEL_ID\",\"name\":\"$SERVICE_NAME\",\"weightsPath\":\"$WEIGHTS\",\"backend\":\"$BACKEND\",\"device\":$DEVICE,\"threads\":$THREADS,\"executableId\":\"$EXECUTABLE_ID\"}" \
...
for ((i = 0; i < WAIT_SECONDS; i++)); do
  line="$(instance_line || true)"
  if [[ -z "$line" ]]; then
    fatal "Breeze instance disappeared while waiting for READY"
  fi
  read -r instance_id status child_port <<<"$line"
  case "$status" in
    READY) break ;;
...
log "Warming Breeze with a short speech request (READY alone does not prove VRAM loading)"
rm -f "$WARMUP_WAV"
...
[[ -s "$WARMUP_WAV" ]] || fatal "warm-up output is empty: $WARMUP_WAV"
[[ "$(dd if="$WARMUP_WAV" bs=1 count=4 2>/dev/null)" == RIFF ]] \
  || fatal "warm-up output is not a RIFF/WAV file: $WARMUP_WAV"
